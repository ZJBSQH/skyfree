...
describe('AgentWorkspace', () => {
  ...
  it('disables generation until a story idea is entered', () => {
    ...
    expect((screen.getByRole('button', { name: '开始创作' }) as HTMLButtonElement).disabled).toBe(true)
  })

  it('emits a trimmed story idea for generation', async () => {
    const { emitted } = renderWorkspace()

    await fireEvent.update(screen.getByRole('textbox', { name: '创作灵感' }), '  A city in the clouds  ')
  ...
  it('shows the synchronous running state without allowing input changes', () => {
    ...
    expect(screen.getByText('智能体正在协作')).toBeTruthy()
    expect(screen.getByText('已用时 01:05')).toBeTruthy()
    expect((screen.getByRole('textbox', { name: '创作灵感' }) as HTMLTextAreaElement).disabled).toBe(true)
    expect(screen.getByText('AgentSky 创作流程')).toBeTruthy()
  })
  ...
  it('shows an error and allows the failed request to be retried', async () => {
    const { emitted } = renderWorkspace({
      status: 'failed',
      errorMessage: '创作失败，请稍后重试'
    })

    expect(screen.getByRole('alert').textContent).toContain('创作失败，请稍后重试')
